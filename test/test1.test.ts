...
describe('P', () => {
    ...
    describe('addition', () => {
        let randomnumbers = new Array(300).fill(0).map(()=>Math.random()*20-10);
        for (let i = 0; i<100; i++) {
            ...
            it(`addition${i}`, ()=>expect(new PowiainaNum(randomnumbers[a]).add(randomnumbers[b]).toNumber())
            .toBeCloseTo(randomnumbers[a]+randomnumbers[b], 7));
        }
    })
    describe('multiply', () => {
        let randomnumbers = new Array(300).fill(0).map(()=>Math.random()*1e16-2e16);
        for (let i = 0; i<100; i++) {
            let a = Math.floor(Math.random()*300)
            let b = Math.floor(Math.random()*300)
            if (b==a && a != 0) {
            ...
            it(`multiply${i}`, ()=>expect(new PowiainaNum(randomnumbers[a]).mul(randomnumbers[b]).toNumber())
